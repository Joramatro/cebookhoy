...
public class PopupVerifyServlet extends HttpServlet {
    ...
    RelyingParty _relyingParty = RelyingParty.getInstance().addListener(
	...
	if ("true".equals(request.getParameter("logout"))
		|| "true".equals((String) request.getAttribute("logout"))) {
	    _relyingParty.invalidate(request, response);
	    request.getSession().setAttribute(OpenIdUser.ATTR_NAME, null);
	...
	try {
		...
		if (user.isAuthenticated()
			|| (user.isAssociated()
				&& RelyingParty.isAuthResponse(request) && _relyingParty
